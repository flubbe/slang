...
TEST(parser, sorted_ast)
{
    {
        const std::string test_input =
          "fn f() -> void { let a: i32 = 1+2+3+4+5; }";
        ...
        std::vector<slang::ast::expression*> sorted_ast = parser.get_sorted_ast();
        ASSERT_EQ(sorted_ast.size(), expected.size());

        for(std::size_t i = 0; i < sorted_ast.size(); i++)
        {
            EXPECT_EQ(sorted_ast[i]->to_string(), expected[i]);
        }
    }
}
